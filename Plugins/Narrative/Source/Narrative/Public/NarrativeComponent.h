...
public:
	...
	static FNarrativeUpdate BeginQuest(const TSubclassOf<class UQuest>& QuestClass, TArray<FName> StartFromIDs)
	{
		FNarrativeUpdate Update;
		Update.UpdateType = EUpdateType::UT_BeginQuest;
		Update.QuestClass = QuestClass;
		for (FName Name : StartFromIDs)
		{
			Update.Payload.Append(Name.ToString() + TEXT(";"));
		}
		return Update;
	};

	//Tell a client to restart one of its quests 
	static FNarrativeUpdate RestartQuest(const TSubclassOf<class UQuest>& QuestClass, TArray<FName> StartFromIDs)
	{
		FNarrativeUpdate Update;
...
public:
	...
	TArray<TSubclassOf<UDialogue>> PlayedDialogues;

	UPROPERTY(EditAnywhere, Category = "Quests")
	bool bIsEnableLoadingStatus = false;
...
public:
	...
	*/
	...
	virtual class UQuest* BeginQuest(TSubclassOf<class UQuest> QuestClass, TArray<FName> StartFromIDs, TArray<FName> BranchToActivate);
	...
	*/
	...
	virtual bool BeginDialogue(TSubclassOf<class UDialogue> Dialogue, class AActor* DefaultNPCAvatar, class AActor* Instigator, FName StartFromID = NAME_None);
