...
void UQuestGraph::PinRewired(UQuestGraphNode* Node, UEdGraphPin* Pin)
{
	...
	else
	{
		...
		{
			for (int Index = 0; Index < Pin->LinkedTo.Num(); Index++)
			{
				UQuestGraphNode_Action* ParentTo = Cast<UQuestGraphNode_Action>(Pin->LinkedTo[Index]->GetOwningNode());
				FromState->State->InputBranches.AddUnique(ParentTo->Branch);
			}
		}
		else if (UQuestGraphNode_Action* FromAction = Cast<UQuestGraphNode_Action>(Node))
		{
			for (int Index = 0; Index < Pin->LinkedTo.Num(); Index++)
			{
				UQuestGraphNode_State* ParentTo = Cast<UQuestGraphNode_State>(Pin->LinkedTo[Index]->GetOwningNode());
				FromAction->Branch->ParentStates.Add(ParentTo->State);
			}
		}
	}
}
