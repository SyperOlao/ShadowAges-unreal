...
void FQuestGraphEditor::Compile()
{
	FBlueprintEditor::Compile();

	UQuestGraph* MyGraph = Cast<UQuestGraph>(QuestBlueprint->QuestGraph);
	for (UEdGraphNode* EdGraphNode : MyGraph->Nodes)
	{
		if (UQuestGraphNode_Action* GraphNodeAction = Cast<UQuestGraphNode_Action>(EdGraphNode))
		{
			UEdGraphPin* InputPin = GraphNodeAction->GetInputPin();
			UEdGraphPin* OutputPin = GraphNodeAction->GetOutputPin();

			GraphNodeAction->Branch->ParentStates.Empty();
			if (OutputPin && OutputPin->LinkedTo.Num() > 0)
			{
				if (UQuestGraphNode_State* StateNode = Cast<UQuestGraphNode_State>(OutputPin->LinkedTo[0]->GetOwningNode()))
				{
					GraphNodeAction->Branch->DestinationState = StateNode->State;
				}
			}

			if (InputPin)
			{
				for (int Index = 0; Index < InputPin->LinkedTo.Num(); ++Index)
				{
					UEdGraphPin* LinkedPin = InputPin->LinkedTo[Index];
					if (!LinkedPin)
					{
						continue;
					}

					if (UQuestGraphNode_State* StateNode = Cast<UQuestGraphNode_State>(LinkedPin->GetOwningNode()))
					{
						GraphNodeAction->Branch->ParentStates.Add(StateNode->State);
					}
				}
			}
		}

		if (UQuestGraphNode_State* GraphNodeState = Cast<UQuestGraphNode_State>(EdGraphNode))
		{
			UEdGraphPin* InputPin = GraphNodeState->GetInputPin();
			UEdGraphPin* OutputPin = GraphNodeState->GetOutputPin();

			GraphNodeState->State->Branches.Empty();
			GraphNodeState->State->InputBranches.Empty();

			if (OutputPin)
			{
				for (int Index = 0; Index < OutputPin->LinkedTo.Num(); ++Index)
				{
					UEdGraphPin* LinkedPin = OutputPin->LinkedTo[Index];
					...
					if (UQuestGraphNode_Action* ActionNode = Cast<UQuestGraphNode_Action>(LinkedPin->GetOwningNode()))
					{
						GraphNodeState->State->Branches.Add(ActionNode->Branch);
					}
				}
			}
			...
			if (InputPin)
			{
				for (int Index = 0; Index < InputPin->LinkedTo.Num(); ++Index)
				{
					...
					if (UQuestGraphNode_Action* ActionNode = Cast<UQuestGraphNode_Action>(LinkedPin->GetOwningNode()))
					{
						GraphNodeState->State->InputBranches.Add(ActionNode->Branch);
					}
				}
			}
		}
...
void FQuestGraphEditor::CopySelectedNodes()
{
	FBlueprintEditor::CopySelectedNodes();

	/*
	TArray<UObject*> CopyArray = GetSelectedNodes().Array();
	for (UObject* Object : CopyArray)
	{
		if (UQuestGraphNode_Action* Action = Cast<UQuestGraphNode_Action>(Object))
		{
			if (Action->Branch != nullptr)
			{
				UObject* NewBranch = DuplicateObject(Action->Branch, Action);
				Action->Branch = Cast<UQuestBranch>(NewBranch);
			}
		}
		else if (UQuestGraphNode_State* StateNode = Cast<UQuestGraphNode_State>(Object))
		{
			if (StateNode->State != nullptr)
			{
				UObject* NewState = DuplicateObject(StateNode->State, StateNode);
				StateNode->State = Cast<UQuestState>(NewState);
			}
		}
	}*/
}

void FQuestGraphEditor::PasteNodesHere(UEdGraph* DestinationGraph, const FVector2f& GraphLocation)
{
	// Find the graph editor with focus
	FBlueprintEditor::PasteNodesHere(DestinationGraph, GraphLocation);

	TArray<UObject*> CurrentlySelectedNodes = GetSelectedNodes().Array();

	for (UObject* Object : CurrentlySelectedNodes)
	{
		if (UQuestGraphNode* Node = Cast<UQuestGraphNode>(Object))
		{
			if (UQuestGraphNode_Action* Action = Cast<UQuestGraphNode_Action>(Object))
			{
				...
				if (Action->Branch != nullptr)
				{
					UQuestBranch* OldBranch = Action->Branch;
					UObject* Outer = OldBranch->GetOuter();

					UObject* NewBranchObject = DuplicateObject(OldBranch, Action);
					Action->Branch = Cast<UQuestBranch>(NewBranchObject);
					QuestBlueprint->QuestTemplate->Branches.Add(Action->Branch);
					Action->Branch->OwningQuest = QuestBlueprint->QuestTemplate;
					Action->QuestNode = Action->Branch;
					Action->Branch->EnsureUniqueID();

					if (Action->Branch != nullptr && OldBranch != nullptr)
					{
						// Duplicate Conditions
						Action->Branch->Conditions.Empty();
						for (UNarrativeCondition* Condition : OldBranch->Conditions)
						{
							if (Condition != nullptr)
							{
								UNarrativeCondition* NewCondition = DuplicateObject(Condition, Condition->GetOuter());
								Action->Branch->Conditions.Add(NewCondition);
							}
						}

						// Duplicate Events
						Action->Branch->Events.Empty();
						for (UNarrativeEvent* Event : OldBranch->Events)
						{
							if (Event != nullptr)
							{
								UNarrativeEvent* NewEvent = DuplicateObject(Event, Event->GetOuter());
								Action->Branch->Events.Add(NewEvent);
							}
						}

						// Duplicate QuestTasks
						Action->Branch->QuestTasks.Empty();
						for (UNarrativeTask* Task : OldBranch->QuestTasks)
						{
							if (Task != nullptr)
							{
								UNarrativeTask* NewTask = DuplicateObject(Task, Task->GetOuter());
								Action->Branch->QuestTasks.Add(NewTask);
							}
						}
					}
				}
			...
			else if (UQuestGraphNode_State* StateNode = Cast<UQuestGraphNode_State>(Object))
			{
				...
				if (StateNode->State != nullptr)
				{
					UQuestState* OldState = StateNode->State;
					UObject* NewStateObject = DuplicateObject(OldState, StateNode);
					StateNode->State = Cast<UQuestState>(NewStateObject);
					QuestBlueprint->QuestTemplate->States.Add(StateNode->State);
					StateNode->State->OwningQuest = QuestBlueprint->QuestTemplate;
					StateNode->QuestNode = StateNode->State;

					StateNode->State->EnsureUniqueID();

					if (StateNode->State != nullptr && OldState != nullptr)
					{
						// Duplicate Conditions
						StateNode->State->Conditions.Empty();
						for (UNarrativeCondition* Condition : OldState->Conditions)
						{
							if (Condition != nullptr)
							{
								UNarrativeCondition* NewCondition = DuplicateObject(Condition, Condition->GetOuter());
								StateNode->State->Conditions.Add(NewCondition);
							}
						}

						// Duplicate Events
						StateNode->State->Events.Empty();
						for (UNarrativeEvent* Event : OldState->Events)
						{
							if (Event != nullptr)
							{
								UNarrativeEvent* NewEvent = DuplicateObject(Event, Event->GetOuter());
								StateNode->State->Events.Add(NewEvent);
							}
						}
					}
				}
