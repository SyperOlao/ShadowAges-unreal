...
void FDialogueGraphEditor::PasteNodesHere(class UEdGraph* DestinationGraph, const FVector2f& GraphLocation)
{
	...
	{
		FBlueprintEditor::PasteNodesHere(DestinationGraph, FVector2f(GraphLocation.X, GraphLocation.Y));
	}
}
...
void FDialogueGraphEditor::Dialogue_PasteNodes()
{
	...
	if (TSharedPtr<SGraphEditor> CurrentGraphEditor = UpdateGraphEdPtr.Pin())
	{
		Dialogue_PasteNodesHere(CurrentGraphEditor->GetPasteLocation2f());
	}
}

void FDialogueGraphEditor::Dialogue_PasteNodesHere(const FVector2f& Location)
{
	...
	else
	{
		...
		*/
		...
		FVector2D PasteLoc = FVector2D(Location.X, Location.Y);
